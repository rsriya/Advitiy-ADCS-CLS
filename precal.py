...
m_LLA = np.genfromtxt('LLA.csv',delimiter=",")	#FLat and Lon in degrees and altitude in m (check frames.latlon for details)
'''
Column 1 : time data
	   2 : Latitude
	   3 : Longitude
...
for k in range(N):
	T = m_mag_ned[k,0]
	v_mag_ecef = fs.ned2ecef(m_mag_ned[k,1:4].copy(),m_LLA[k,1],m_LLA[k,2]) 
	#Converting NED frame components of magnetic field to ECEF
	
	m_mag_i[k,0] = T
	m_mag_i[k,1:4] = fs.ecef2ecif(v_mag_ecef.copy(),T)                      #calling function of frame which does the conversion from ECEF to ECIF 
